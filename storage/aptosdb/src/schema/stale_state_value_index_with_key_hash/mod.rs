...
use crate::schema::{
    ensure_slice_len_eq, ensure_slice_len_gt, STALE_STATE_VALUE_INDEX_WITH_KEY_HASH_CF_NAME,
};
use anyhow::Result;
...
use aptos_types::{
    state_store::state_value::StaleStateValueWithKeyHashIndex, transaction::Version,
};
use byteorder::{BigEndian, ReadBytesExt, WriteBytesExt};
use std::{io::Write, mem::size_of};

define_schema!(
    StaleStateValueIndexWithKeyHashSchema,
    StaleStateValueWithKeyHashIndex,
    (),
    STALE_STATE_VALUE_INDEX_WITH_KEY_HASH_CF_NAME
);

impl KeyCodec<StaleStateValueIndexWithKeyHashSchema> for StaleStateValueWithKeyHashIndex {
...
impl ValueCodec<StaleStateValueIndexWithKeyHashSchema> for () {
...
impl SeekKeyCodec<StaleStateValueIndexWithKeyHashSchema> for Version {
